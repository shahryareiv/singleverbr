...
#'
#' @examples
sv_test_and_train_sets <- function(
  data.df,
  response.name,
  ...
  if(NROW(response.name) == 1){
    train.response.c <- data.df[train.v,] %>% select(response.name) %>% unlist()#(data.df[train.v,  names(data.df) %in% response.name])[[1]]#[[1]]#!(names(data.df) %in% response.name)
    test.response.c <- data.df[test.v,] %>% select(response.name) %>% unlist()#(data.df[test.v, names(data.df) %in% response.name])[[1]]#[[1]]#!(names(data.df) %in% response.name)
  }else if(NROW(response.name) > 1){
    train.response.c <- data.df[train.v,] %>% select(response.name)#(data.df[train.v,  names(data.df) %in% response.name])#[[1]]#!(names(data.df) %in% response.name)
    test.response.c <- data.df[test.v,] %>% select(response.name)#(data.df[test.v, names(data.df) %in% response.name])#[[1]]#!(names(data.df) %in% response.name)
  }else{
    stop('Wrong number of response.name!')
  }

  # after dummifiying should it be numeric or categorial?
  # if numeric
  if (is.list(train.response.c) & is.list(test.response.c)){
    train.response.c <- data.matrix(train.response.c)
    test.response.c <- data.matrix(test.response.c)
